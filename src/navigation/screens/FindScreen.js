...
export default function FindScreen({ navigation }) {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      // API 호출
      const response = await fetch(`${SERVER_URL}${encodeURIComponent(query)}`, {
        method: "GET",
        headers: {
      ...
      if (response.status === 200) {
        ...
        Alert.alert("서버 오류", "서버에서 요청을 처리할 수 없습니다. 나중에 다시 시도해주세요.");
      } else {
  ...
  const renderItem = ({ item }) => {
    return (
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate("Profileinfo", { nickname: item.nickname })}
      >
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          data={users}
          keyExtractor={(item) => item.id ? item.id.toString() : item.nickname}
