  ...
  const DeleteLike = async (capsuleLikeId) => {
    try {
      ...
      // API 요청 전송
      const response = await fetch(`${base_url}/api/capsules/likes/${capsuleLikeId}`, {
        method: 'DELETE',
        headers: {
  ...
  const getCapsuleLikeId = async (capsuleId) => {
    try {
      ...
      // 3. API 호출
      const response = await fetch(`${base_url}/api/capsules/${capsuleId}/likes`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      if (result && result.data && Array.isArray(result.data)) {
        ...
        const userLike = result.data.find(like => like.profileId === parseInt(profileId));
  ...
  const GetLikeCount = async (capsuleId) => {
    try {
      ...
      if (result.status === "OK" && Array.isArray(result.data)) {
        ...
        const userLiked = result.data.some(user => user.profileId === parseInt(profileId));
